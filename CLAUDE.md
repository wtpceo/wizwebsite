...
### 11. 엠대시(—)는 제목·소제목에만 쓴다
2026-08-20 전수 점검에서 가이드 글에 엠대시가 264개 있었다. 검색엔진이 벌점을 주는
근거는 없지만, **완결된 문장 뒤에 대시를 붙여 설명을 잇는 형태**는 국내 마케터·사장님
사이에서 AI 생성글 신호로 통한다. "직접 측정한 실측 기록"이라는 우리 포지션과 정면으로
충돌한다.
- ✅ 허용: **제목·H2·H3의 부제 구분** (`GEO 비용 — 견적서 뜯어보는 법`), 목록 라벨 구분
  (`<strong>양(볼륨)</strong> — 리뷰가 충분히 쌓여 있는가`)
- ❌ 금지: **문장 종결어미 뒤** (`~습니다 — 그래서…`). 마침표로 끊고 새 문장으로 쓴다
- JSX에서 `—{" "}`를 마침표로 바꿀 때는 `.{" "}`로. `. {" "}`는 이중 공백이 된다

### 12. 점검 완료 항목 (2026-08-20 실측 — 재점검 불필요)
...
| 사이트맵 lastmod | 2026-08-20 수정 — 전 URL 동일값(빌드시각) → 실제 발행일 17종으로 분리 |

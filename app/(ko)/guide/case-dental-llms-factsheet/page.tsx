      ...
      >
        ...
        <div className="mt-6 overflow-x-auto rounded-xl border border-slate-200">
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="bg-slate-50"><Th>겉으로 보인 증상</Th><Th>실제 원인</Th><Th>대응과 결과</Th></tr>
            </thead>
            <tbody>
              {[
                ["페이지가 느리고 가끔 503", "화면 저장(캐시)이 한 번도 동작한 적이 없었습니다. 저장 여부를 판정하는 조건이 늘 거짓이었습니다.", "판정 기준 교체. 첫 바이트 3.85초에서 0.60~0.70초로"],
                ["오픈 다음 날 디자인이 사라진 화면", "배포할 때마다 스타일 파일 이름이 바뀌는데, 저장된 옛 화면이 사라진 파일을 부르고 있었습니다. 제작 측 PC에서는 정상으로 보여 몇 시간 동안 몰랐습니다.", "배포 → 캐시 비우기 → 캐시 채우기 3단계로 고정하고 자동 수행"],
                ["“글을 지웠는데 사이트에 남아 있다”", "관리자 저장 시 캐시 삭제가 일주일 동안 한 번도 성공하지 못했는데, 실패를 조용히 삼키고 있어 아무도 몰랐습니다.", "실패를 로그로 남기도록 바꾸고 실시간 로그로 검증"],
                ["검색엔진이 “페이지를 찾을 수 없음” 보고", "블로그 글 목록을 네이버 RSS에서 실시간으로 읽고 있었는데 RSS는 최신 50건만 줍니다. 새 글을 올릴 때마다 가장 오래된 글이 사이트에서 사라지는 구조였습니다.", "글 정보를 자체 DB에 보관. 사라졌던 6건 복구"],
                ["같은 글이 4개 주소로 중복", "4개 언어판이 모두 한국어 원문 그대로였는데 각자 대표 주소를 주장하고 있었습니다. 검색엔진 입장에서는 중복 콘텐츠입니다.", "대표 주소를 한국어판으로 통일. 사이트맵 308 → 167 → 120건 (중복 등록을 걷어낸 것)"],
                ["외부 GEO 진단서가 수십 건을 지적", "1차 7건 중 3건, 2차 24건 중 8건이 오탐이었습니다. 의도된 색인 차단, 표준에 맞는 장식 아이콘 처리, 띄어쓰기 차이를 다른 주제로 읽은 경우 등.", "지적마다 실제 응답을 직접 열어 확인한 뒤 고칠 것과 반박할 것을 구분"],
                ["진료 페이지 내용이 부실하다는 지적", "분량이 아니라 빠진 단계가 문제였습니다. 정의와 원인은 있는데 “어떻게 진단하고, 누구에게, 언제 하며, 끝난 뒤 무엇을 하는지”가 없었습니다. 환자가 실제로 검색하는 것은 그 네 가지입니다.", "21개 페이지에 그 단계를 추가. 본문 275~1,192자 → 758~1,585자"],
                ["상호에 지역명을 붙일지 논쟁", "“○○치과의원 지역명”처럼 붙이면 노출에 유리하다는 통념. 그러나 개설신고 명칭과 달라 의료법·플랫폼 정책상 위험이 있습니다.", "실측으로 대응. 해당 키워드 상위 16곳 중 3곳은 지역명 없이 상위였고, 이 병원은 지역명을 넣고도 14위였습니다. 상호로 순위가 설명되지 않았습니다"],
              ].map((r) => (
                <tr key={r[0]} className="border-t border-slate-100"><Td strong>{r[0]}</Td><Td>{r[1]}</Td><Td>{r[2]}</Td></tr>
              ))}
            </tbody>
        ...
        <ul>
          <CheckItem><strong>1. 본문을 자사 도메인에 둡니다.</strong> 플랫폼은 AI 크롤러를 막습니다. 거기에만 있으면 AI에게는 없는 콘텐츠입니다.</CheckItem>
          <CheckItem><strong>2. 서버에서 완성된 HTML을 보냅니다.</strong> 자바스크립트로 나중에 그리면 크롤러 상당수가 본문을 못 읽습니다. 이걸 놓치면 이후 작업이 전부 무효입니다.</CheckItem>
          <CheckItem><strong>3. 항상 빠르게 열리게 만듭니다.</strong> 느리거나 오류가 나면 읽히기 전에 끝납니다. 콘텐츠보다 먼저입니다.</CheckItem>
          ...
          <CheckItem><strong>6. 사실의 표기를 한 글자까지 통일합니다.</strong> 상호·주소·전화가 사이트, 구조화 데이터, 외부 채널에서 조금이라도 다르면 같은 업체로 묶이지 않습니다.</CheckItem>
          <CheckItem><strong>7. 중복 주소를 정리합니다.</strong> 같은 글이 여러 주소에 있으면 평가가 갈립니다. 대표 주소를 하나로 정합니다.</CheckItem>
          <CheckItem><strong>8. &ldquo;판단에 쓰이는 값&rdquo;을 본문에 넣습니다.</strong> 일반론은 인용되지 않습니다. 급여 기준, 요일별 시간, 자격 표기처럼 검증 가능한 값이 인용됩니다.</CheckItem>
        </ul>

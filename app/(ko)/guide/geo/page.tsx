...
export const metadata: Metadata = {
  ...
  openGraph: {
    images: ["/covers/geo.jpg"], title: TITLE, description: DESC, url: URL, type: "article",
  },
}
...
const faqJsonLd = {
  ...
  mainEntity: [
    ...
    },
    {
      ...
      acceptedAnswer: {
        "@type": "Answer",
        text: "다섯 단계로 진행합니다. ① AI가 우리를 어떻게 답하는지 현재 상태를 실측하고 ② AI 크롤러가 홈페이지를 읽을 수 있는지 기술 점검을 하고 ③ 홈페이지·지도·디렉토리의 정보를 하나로 통일하고 ④ 고객의 실제 질문에 답하는 콘텐츠와 구조화 데이터를 설계하고 ⑤ 매주 같은 기준으로 재측정합니다. 순서가 중요합니다. 크롤러가 막혀 있는 상태에서 콘텐츠부터 늘리는 것이 가장 흔한 실수입니다.",
      },
    },
    {
      "@type": "Question",
    ...
    },
    {
      ...
      acceptedAnswer: {
        "@type": "Answer",
        text: "정보 정합성 정리, 리뷰 관리, FAQ 작성 같은 콘텐츠 영역은 직접 할 수 있습니다. 반면 크롤러 접근 점검, 구조화 데이터 설계, 색인 관리 같은 기술 영역과 매주 측정·리포팅은 손이 많이 갑니다. 대행을 검토한다면 계약 전 진단을 먼저 주는지, 노출을 보장한다고 하지 않는지, 계약 종료 후 홈페이지와 콘텐츠가 누구 소유로 남는지를 확인하세요.",
      },
    },
  ],
}
  ...
  return (
    <>
      ...
      >
        ...
        <h2>GEO 뜻과 GEO 최적화의 범위</h2>
        <p>
          GEO는 영어 Generative Engine Optimization의 앞 글자를 딴 말이고, 국내에서는
          &lsquo;지오&rsquo;로 읽습니다. 검색 순위를 올리는 작업(SEO)과 목표가 다릅니다. GEO는
          순위가 아니라 <strong>AI가 만든 답변 안에 우리 이름이 들어가는가</strong>를 봅니다.
        </p>
        <p>
          GEO 최적화라고 부르는 일에는 보통 네 가지가 들어갑니다. 첫째 AI 크롤러가 우리 사이트를
          읽을 수 있게 여는 일, 둘째 브랜드 정보를 채널마다 같게 맞추는 일, 셋째 AI가 인용하기 좋은
          형태로 사실을 정리해 쌓는 일, 넷째 엔진별로 얼마나 언급되는지 반복 측정하는 일입니다.
          가게를 운영하시는 분이라면 쉬운 말로 다시 쓴{" "}
          <a href="/guide/what-is-geo">자영업 AI 검색 최적화 입문</a>부터 보셔도 됩니다.
        </p>
        ...
        <ol>
          ...
          <CheckItem>
            <strong>5단계 · 재측정과 반복</strong>: 매주 같은 기준으로 다시 재서 변화를 숫자로
        ...
        <p>
          ...
          기술 영역과 매주 반복되는 측정·리포팅. 특히 병원처럼 의료광고법을 함께 지켜야 하는

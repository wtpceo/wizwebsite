...
import { ChevronRight, BookOpen, Sparkles, MapPin } from "lucide-react"
...
export const metadata: Metadata = {
  ...
  description:
    "AI 검색 최적화(GEO), 네이버 플레이스, 자영업 마케팅 실전 가이드. 위즈더플래닝이 7,000여 광고주와 쌓은 노하우를 사장님 눈높이로 정리했습니다.",
  alternates: { canonical: "/guide" },
  openGraph: {
    title: "마케팅 가이드 | 위즈더플래닝",
    description:
      "AI 검색 최적화(GEO), 네이버 플레이스, 자영업 마케팅 실전 가이드 모음.",
          ...
          <div className="space-y-5">
            {GUIDES.map((g) => (
              <Link
                key={g.href}
                href={g.href}
                className="group flex items-start gap-5 rounded-2xl border border-gray-200 bg-white p-6 transition-all hover:border-emerald-500/40 hover:shadow-xl hover:shadow-emerald-500/[0.07] md:p-8"
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <h2 className="mt-1.5 text-lg font-extrabold tracking-tight text-gray-900 group-hover:text-emerald-700 md:text-xl">
                    {g.title}
                  </h2>
